...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (cmd != NULL) {
		if (!strncmp(cmd, "bootloader", 10)) {
			...
			//qpnp_pon_store_extra_reset_info(RESET_EXTRA_REBOOT_BL_REASON, RESET_EXTRA_REBOOT_BL_REASON);   // Motorola 
		...
		__raw_writel(0x77665505, restart_reason);    // Motorola // https://github.com/MotorolaMobilityLLC/kernel-msm/commit/74a038a1fe0e77cd80e3fc2d18e7da5e25e56f03 
		//qpnp_pon_store_extra_reset_info(RESET_EXTRA_PANIC_REASON, RESET_EXTRA_PANIC_REASON);    // Motorola 
	} else {
		__raw_writel(0x77665501, restart_reason);
	}
